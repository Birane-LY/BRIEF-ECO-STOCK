...
class ProductViewSet(viewsets.ModelViewSet):
  ...
  def move_product(self, request, pk=None):
      ...
      try:
          ...
          return Response(
              {"error": "Entrepôt ciblé non trouvé."}, 
  ...
  def transfer(self, request, pk=None):
      """ Transfer the product to a different warehouse. """
      product = self.get_object()
      target_warehouse_id = request.data.get('warehouse_id')
      if not target_warehouse_id:
          return Response({"error": "L'ID de l'entrepôt est obligatoire."}, status=status.HTTP_400_BAD_REQUEST)
      try:
          target_warehouse = Warehouse.objects.get(id=target_warehouse_id)
          product.warehouse = target_warehouse
          product.save()
          ...
          return Response({"error": "Entrepôt ciblé non trouvé."}, status=status.HTTP_404_NOT_FOUND)

  def get_permissions(self):
      if self.action in ['list', 'retrieve']:
          permission_classes = [AllowAny]
      elif self.action in ['create', 'move_product', 'audit', 'transfer']:
          permission_classes = [IsAuthenticated]
      else:
          permission_classes = [IsAdminUser]
      return [permission() for permission in permission_classes]
